...
(define-private (is-valid-status (status uint))
  (or (is-eq status STATUS-PLANNING)
      (or (is-eq status STATUS-DRILLING)
          (or (is-eq status STATUS-TESTING)
...
(define-public (update-status 
  (project-id uint) 
  (new-status uint))
  (let ((project (unwrap! (map-get? project-registry { project-id: project-id }) ERR-PROJECT-NOT-FOUND))
        (current-status-info (unwrap! (map-get? project-status { project-id: project-id }) ERR-PROJECT-NOT-FOUND)))
    
    ;; Validate caller is project owner or contract owner
    (asserts! (or (is-project-owner project-id tx-sender) 
                  (is-contract-owner tx-sender)) ERR-UNAUTHORIZED)
    
    ;; Validate status
    (asserts! (is-valid-status new-status) ERR-INVALID-STATUS)
    
    ;; Check if already completed
    (asserts! (not (is-eq (get status current-status-info) STATUS-COMPLETED)) ERR-ALREADY-COMPLETED)
...
(define-public (add-milestone
  ...
  (let ((project (unwrap! (map-get? project-registry { project-id: project-id }) ERR-PROJECT-NOT-FOUND)))
    ...
    (map-set project-milestones
      { project-id: project-id, milestone: milestone-id }
      {
...
(define-public (complete-milestone
  ...
  (milestone-id uint))
  (let ((project (unwrap! (map-get? project-registry { project-id: project-id }) ERR-PROJECT-NOT-FOUND))
        (milestone-info (unwrap! (map-get? project-milestones { project-id: project-id, milestone: milestone-id }) ERR-PROJECT-NOT-FOUND)))
...
(define-read-only (get-milestone-info (project-id uint) (milestone-id uint))
  (match (map-get? project-milestones { project-id: project-id, milestone: milestone-id })
